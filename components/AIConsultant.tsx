...
const AIConsultant: React.FC<AIConsultantProps> = ({ isOpen, onClose, contextData }) => {
  ...
  return (
    <div className="fixed inset-0 lg:inset-y-0 lg:left-auto lg:right-0 lg:w-96 bg-black border-l border-[#1a1a1a] shadow-2xl z-[60] flex flex-col animate-in slide-in-from-right duration-500">
      <div className="p-5 lg:p-6 border-b border-[#1a1a1a] flex justify-between items-center bg-[#0b0b0b]">
        <div className="flex items-center gap-2">
          <Sparkles className="text-[#D4AF37]" size={20} />
          <h2 className="font-black text-[#D4AF37] uppercase text-xs lg:text-sm tracking-widest">GiroChef AI</h2>
        </div>
        <button onClick={onClose} className="p-2 text-slate-500 hover:text-[#D4AF37] hover:bg-[#1a1a1a] rounded-xl transition-all">
          <X size={24} />
        </button>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[90%] lg:max-w-[85%] rounded-2xl p-4 text-sm leading-relaxed relative group ${
              msg.role === 'user' 
              ...
              <div className="prose prose-invert prose-sm max-w-none">
                {msg.content.split('\n').map((line, j) => (
                  <p key={j} className="mb-2 last:mb-0 break-words">{line}</p>
                ))}
              </div>
            </div>
          </div>
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-[#1a1a1a] border border-[#1a1a1a] rounded-2xl rounded-tl-none p-4 flex items-center gap-3">
              ...
              <span className="text-[10px] text-slate-500 italic uppercase tracking-tighter">Processando Inteligência...</span>
            </div>
          </div>
        )}
      </div>

      <div className="p-4 lg:p-6 border-t border-[#1a1a1a] bg-[#0b0b0b]">
        ...
        <p className="text-[8px] lg:text-[9px] text-slate-600 mt-4 text-center font-bold uppercase tracking-[0.2em]">
